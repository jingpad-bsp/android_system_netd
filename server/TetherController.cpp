...
int TetherController::enableNat(const char* intIface, const char* extIface) {
    ...
    std::string iIface = intIface;
    std::string eIface = extIface;
    const char sys_dev_pamu3[] = "/sys/devices/platform/soc/soc:ap-ahb/21100000.pamu3/pamu3_netid";
    if(iIface.find("usb") >= 0){
        ALOGD("usb is tethering");
        char numOfIface[] = {eIface.at(eIface.length()-1)};
        ALOGD("numOfIface is : %c",numOfIface[0]);
        WriteStringToFile(numOfIface,sys_dev_pamu3);
    } 
