...
#define  CMD_TYPE_PPPUP           (5)
#define  CMD_TYPE_PPPD_START      (6)
#define  CMD_TYPE_PPPD_STOP       (7)
#define  CMD_TYPE_END             (8)
...
struct command {
    ...
    char ipv4addr[MAX_ADDR_LENGTH];/*save ipv4 addr for ppp*/
    char *ttyName;//for pppd param
    char local[MAX_ADDR_LENGTH];
    char remote[MAX_ADDR_LENGTH];
...
public:
    ...
    void do_ppproute(struct command *c);
    void do_pppd_start(struct command *c);
    void do_pppd_stop();
